...
export function buildAgentManualCommand(): Command {
  return new Command("agent-manual")
    .description(
      "Generate the Lyt agent manual and (with --install) inject it into agent-runtime global instructions files. Default (--install, no --runtime): auto-detect ALL installed runtimes (~/.claude, ~/.codex, ~/.agents) and inject into each (absent runtimes skipped with a note). Explicit --runtime <one> targets a single runtime; generic always prints to stdout. Hand-curated mental-model + workflows + protocol-notes + auto-injected WHEN-USER-SAYS table from 11 SKILL.md files + curated CLI-verb list. update-path primitive: <!-- lyt-manual v<lyt-version> BEGIN -->...END --> markers (replace-between-markers on re-install; refuse on malformed; --force appends a fresh block on malformed with a visible warning).",
    )
    .option(
      "--runtime <name>",
