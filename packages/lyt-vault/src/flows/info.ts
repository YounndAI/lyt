...
export interface InfoFlowResult {
  ...
  sizeBytes: number;
  fileCount: number;
  // license posture for the vault. `public` is currently always false —
  // @MESH_PUBLIC was removed in fed-v2 Slice 1b (#13); the vault-scoped
  // publish surface is rebuilt in a later layer. `license` is detected from
  // the LICENSE file at the vault root via license-detect heuristic; null
  // when LICENSE absent or unrecognised. `bucket` is the
...
export async function infoVaultFlow(
  ...
  try {
    ...
    // Fed-v2 Slice 1b (#13 DELETE) — @MESH_PUBLIC removed. The `public`
    // flag in licensePosture is now always false (vault-scoped publish surface
    // is rebuilt in a later layer). License id + bucket still derived from the
    // LICENSE file at the vault root.
