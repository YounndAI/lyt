...
// Phase C (UNIT 5) — scaffold-generation version stamp carried in the brand-grade
// map README. AGENTS_MD_TEMPLATE_VERSION is the canonical "scaffold-file-set
// generation" integer (priming.ts:28-32 documents the coupling); reusing it keeps
// a SINGLE bump point for every scaffold-version surface (no 2nd version concept).
...
// Phase C (UNIT 5) — brand-grade, version-stamped map README. The map vault is
// generator-managed (no Figment seed, regenerated each run), so this README is
// itself a generated artifact and the version stamp is a generation marker — NOT
// a user Figment body (SC9 keeps version stamps out of user-authored files; this
// generated documentation file is exempt and EXPECTED to carry the stamp).
function renderReadme(
  ...
  return (
    `# Lyt Pod Map — \`${owner}\`\n\n` +
    `> Generated by \`${GENERATOR_TAG}\` (scaffold v${AGENTS_MD_TEMPLATE_VERSION}) at ${nowIso}.\n` +
    `> ${meshCount} mesh(es), ${vaultCount} vault(s).\n\n` +
    `This is a **Lyt** pod-map vault — a live visualisation of your pod's ` +
    `federation topology. Each note under \`meshes/\` is one mesh; each note under ` +
    `\`vaults/\` is one vault; wikilinks encode the federation edges between them.\n\n` +
    `**Open this vault in Obsidian and switch to graph view** to render the topology ` +
    `natively. Install the bundled Pod Manager community plugin ` +
    `(\`.obsidian/plugins/lyt-pod-manager/\`) for mesh-boundary colouring and ` +
    `🔒 read-only badges.\n\n` +
    `Lyt only writes the files it generates; this whole vault is regenerated on each ` +
    `\`lyt init --wizard\` re-run or \`/lyt-sync\` regen hook.\n\n` +
    `_Generator-managed (\`vault.kind: pod-map\` + \`writable: false\`). Do not edit ` +
    `notes here — they are overwritten on every regen. **Do not place new files under ` +
    `\`meshes/\` or \`vaults/\`** — orphan files are silently removed on the next regen._\n\n` +
