...
- **`vault.visibility`** (handler-gated) — previews a public/private flip for one own vault: reports the current GitHub repository visibility, the `@FED_VAULT` manifest value, and the exact `lyt-public` topic deltas a flip would apply. It changes nothing — the handler hard-codes `confirmed: false`, so this tool can never apply the flip. Applying it is CLI-only: `lyt vault visibility <vault> --public|--private --yes`.
